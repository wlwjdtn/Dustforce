    ...
    void MovePassengers(Vector3 velocity) {
        ...
        if (velocity.x != 0) {
            ...
            for (int i = 0; i < _HoriRayCount; i++) {
                ...
                RaycastHit2D _Hit = Physics2D.Raycast(rayOrigin, Vector2.right * directionX, rayLength, passengerMask);

                if (_Hit) {
                    if (!movedPassengers.Contains(_Hit.transform)) {
                        movedPassengers.Add(_Hit.transform);
                        float pushX = velocity.x - (_Hit.distance - _SkinWidth) * directionX;
                        float pushY = 0;
