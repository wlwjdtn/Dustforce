...
public class DustgirlMovement: MonoBehaviour {
    ...
    private void Update() {
        ...
        // 
        //else if ((_Controll2D._ColliderInfo._Left || _Controll2D._ColliderInfo._Right) && !_Controll2D._ColliderInfo._Below && _Velocity.y > 0) {
        //    wallSliding = true;
        ...
        // 벽에 붙었을 때 점프
        if (Input.GetKeyDown(KeyCode.Z) && _Velocity.y < 0) {
            if (wallSliding) {
                if (wallDirX == input.x) {
        ...
        // 벽을 타고 있을 때 위쪽 키를 누르고 있으면?
        if (wallSliding && input.y > 0) {
        ...
        _CharacterAnim.WallRunAnim(inputWallRunKey);
    }
}